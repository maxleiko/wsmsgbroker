...
module.exports = function (server, ws, msg) {
    ...
    function send(id) {
        ...
        if (destWs) {
            if (destWs.readyState === WebSocket.OPEN) {
                ...
                server.emit('error', new Error('Unable to send message. Client "'+id+'" disconnected'));
            }
        } else {
    ...
    try {
        ...
        server.emit('error', new Error('Something went wrong while processing "send" (reason: '+err.message+')'));
    }
};
